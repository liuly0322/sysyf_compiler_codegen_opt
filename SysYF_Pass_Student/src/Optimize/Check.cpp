...
template <typename T1, typename... Ts> void writeTs(T1 V1, Ts... Vs) {
    writeInfo(V1);
    writeTs(Vs...);
}
...
void Check::checkFunction(Function *fun) {
    ...
    Verify(fun->get_parent() == module,
           "Function context does not match Module context!", fun);

    // Check function return type.
    Type *retTy = fun->get_return_type();
    Verify(retTy->is_integer_type() || retTy->is_float_type() ||
               retTy->is_void_type(),
           "Functions cannot return aggregate values!", fun);

    // Check the entry node.
    BasicBlock *entry = fun->get_entry_block();
    Verify(entry->get_pre_basic_blocks().empty(),
           "Entry block to function must not have predecessors!", fun);
...
void Check::checkBasicBlock(BasicBlock *bb) {
    ...
    Verify(bb->get_terminator() != nullptr,
           "Basic Block does not have terminator!", bb, fun);

    // Check constraints that this basic block imposes on all of the PHI nodes
    // in it. Collect PHI instructions.
    ...
    for (auto *inst : bb->get_instructions()) {
        ...
        if (inst->is_phi()) {
            Verify(phi_list.count(inst),
                   "PHINode is not grouped at top of basic block!", inst, bb,
                   fun, "\nHere is block content\n", bb->print());
        }
        Verify(
            inst->get_parent() == bb,
            "Basic Block contains an instruction whose parent is not the bb!",
            inst, bb, fun);
    ...
    for (auto *succ_bb : bb->get_succ_basic_blocks()) {
        ...
        Verify(it != preds.end(), "contradictory precursor and successor!", fun,
               "current block", bb->get_name(),
...
void Check::checkPhiInstruction(Instruction *inst) {
    ...
    for (auto *opr : inst->get_operands()) {
        if (dynamic_cast<BasicBlock *>(opr) != nullptr) {
            Verify(preds.count(static_cast<BasicBlock *>(opr)) != 0,
                   "Useless phi operand!", inst, bb, fun);
    ...
    for (auto *val : values) {
        Verify(val->get_type()->get_type_id() ==
                   inst->get_type()->get_type_id(),
...
void Check::checkInstruction(Instruction *inst) {
    ...
    for (auto *opr : inst->get_operands()) {
        ...
        if (func != nullptr) {
            Verify(functions.count(func), "Operand(Function) is not defined!",
    ...
    if (inst->is_br()) {
        ...
        for (auto *opr : inst->get_operands()) {
            ...
            if (block != nullptr) {
                ...
                Verify(it != succ_blocks.end(),
                       "contradictory precursor and successor!", inst, bb, fun,
                       "block", block->get_name(), "not in successor!");
            }
        }
    }
    ...
    if (inst->is_ret()) {
        ...
        if (ret_inst->is_void_ret()) {
            Verify(fun->get_return_type()->is_void_type(),
                   "Wrong function return type!", inst, bb, fun);
        } else {
            Verify(fun->get_return_type()->get_type_id() ==
                       inst->get_operand(0)->get_type()->get_type_id(),
                   "Wrong function return type!", inst, bb, fun);
        }
    }
}
...
void Check::checkCallInstruction(Instruction *inst) {
    ...
    Verify(funcTy->get_return_type()->get_type_id() ==
               inst->get_type()->get_type_id(),
           "Wrong call function return type!", inst, bb, fun);

    // Check nums of parameter.
    Verify(inst->get_num_operand() - 1 == funcTy->get_num_of_args(),
           "The number of parameters is inconsistent!", inst, bb, fun);

    // Check the consistency of parameter types.
    for (auto i = 1U; i < inst->get_num_operand(); i++) {
        Verify(inst->get_operand(i)->get_type()->get_type_id() ==
                   funcTy->get_param_type(i - 1)->get_type_id(),
               "Parameter type conflict!", inst, bb, fun);
    }
}

void Check::checkUse(User *user) {
    // Check that all uses of the instruction/global value, if they are
    ...
    for (auto &use : user->get_use_list()) {
        ...
        if (use_inst != nullptr) {
            Verify(use_inst->get_parent() != nullptr,
                   ...
                   "embedded in a basic block!",
                   user);
            Verify(use_inst->get_operand(use.arg_no_) == user,
                   "Wrong use number!", user, use_inst);
        } else {
            checkFailed(
                "Use of instruction/global value is not an instruction!", user);
            return;
        }
    }
}
