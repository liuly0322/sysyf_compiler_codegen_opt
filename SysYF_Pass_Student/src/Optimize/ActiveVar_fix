    ...
    for (auto *func : this->module->get_functions()) {
        ...
        while (changed) {
            ...
            for (auto *bb : blocks) {
                ...
                for (auto *succ_bb : succ_blocks) {
                    ...
                    for (auto *inst : succ_bb->get_instructions()) {
                        ...
                        for (int i = 0; i < phi_size; i++) {
                            auto *block = dynamic_cast<BasicBlock *>(
                                phi_inst->get_operand(2 * i + 1));
                            if (block != nullptr && block == bb) {
                                auto *active_var = dynamic_cast<Value *>(
                                    phi_inst->get_operand(2 * i));
                                auto *arg_var =
                                    dynamic_cast<Argument *>(active_var);
                                auto *inst_var =
                                    dynamic_cast<Instruction *>(active_var);
                                if ((arg_var != nullptr) ||
                                    (inst_var != nullptr)) {
                ...
                // 将tmp转化为IN，先看是否有修改
                if (tmp_out_bb.size() != IN_list[bb].size())
                    changed = true;
                else {
